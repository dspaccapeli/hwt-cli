...
const execUI = async (apiKey, modelName, prompt, language, autoCopy, fastExec, directOut) => {
	let app;
	//change to init
	app = render(React.createElement(exec, {apiKey, modelName, prompt, language, autoCopy, fastExec, directOut }));
	await app.waitUntilExit();
	process.exit();
}
